    ...
    if (isConversationComment(comment.text)) {
        ...
        if (userName !== '') {
            ...
            if (mentionedUser !== null) {
                ...
                yield NotificationsHandler.sendNotifications(mentionedUser.devices, title, message, user.profilePicture, NOTIFICATION_TYPES.USER_MENTIONED);
            }
        }
    } else {
        var title = String.format(MESSAGES.USER_COMMENTED_TITLE, user.username, app.name);
        var message = comment.text;
        yield NotificationsHandler.sendNotifications(app.createdBy.devices, title, message, user.profilePicture, NOTIFICATION_TYPES.USER_COMMENT);
    }
