...
Co(function* () {
    ...
    var apps = yield App.find({ status: APP_STATUS_FILTER.APPROVED, platform: PLATFORMS.Android }).exec();
    var size = apps.length;

    var nullApps = 0;
    for (var i = 1200; i < size; i++) {
        var app = apps[i];
        var parsedApp = yield DevsHunter.updateAndroidApp(app['package']);
        if (parsedApp == null) {
